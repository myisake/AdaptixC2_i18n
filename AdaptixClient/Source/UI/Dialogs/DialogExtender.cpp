...
void DialogExtender::createUI()
{
    ...
    tableWidget->setHorizontalHeaderItem(2, new QTableWidgetItem(tr("Status" ) ) );
    tableWidget->setHorizontalHeaderItem(3, new QTableWidgetItem(tr("Description" ) ) );
...
void DialogExtender::UpdateExtenderItem(const ExtensionFile &extenderItem) const
{
    for (int row = 0; row < tableWidget->rowCount(); ++row) {
        ...
        if ( item && item->text() == extenderItem.FilePath ) {
            ...
            if ( extenderItem.Enabled ) {
                tableWidget->item(row, 2)->setText(tr("Enable"));
                tableWidget->item(row, 2)->setForeground(QColor(COLOR_NeonGreen));
            }
            else {
                if (extenderItem.Message.isEmpty()) {
                    tableWidget->item(row, 2)->setText(tr("Disable"));
                    tableWidget->item(row, 2)->setForeground(QColor(COLOR_BrightOrange));
                }
                else {
                    tableWidget->item(row, 2)->setText(tr("Failed"));
...
{
    NonBlockingDialogs::getOpenFileName(const_cast<DialogExtender*>(this), tr("Load Script"), "", "AxScript Files (*.axs)",
