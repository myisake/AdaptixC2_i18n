...
bool DialogConnect::checkValidInput() const
{
    if ( lineEdit_Project->text().isEmpty() ) {
        MessageError(tr("Project is empty"));
        return false;
    }
    if ( lineEdit_Host->text().isEmpty() ) {
        MessageError(tr("Host is empty"));
        return false;
    }
    if ( lineEdit_Port->text().isEmpty()  ) {
        MessageError(tr("Port is empty"));
        return false;
    }
    if ( lineEdit_Endpoint->text().isEmpty()  ) {
        MessageError(tr("Endpoint is empty"));
        return false;
    }
    if ( !IsValidURI(lineEdit_Endpoint->text()) ) {
        MessageError(tr("Endpoint is invalid URI (Example: /api/qwe)"));
        return false;
    }
    if ( lineEdit_User->text().isEmpty() ) {
        MessageError(tr("Username is empty"));
        return false;
    }
    if ( lineEdit_Password->text().isEmpty() ) {
        MessageError(tr("Password is empty"));
        return false;
    }

    if( GlobalClient->storage->ExistsProject(lineEdit_Project->text()) && isNewProject ) {
        MessageError(tr("Project already exists"));
        return false;
    }
