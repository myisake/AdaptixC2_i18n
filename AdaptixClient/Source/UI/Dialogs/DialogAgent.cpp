...
void DialogAgent::onButtonGenerate()
{
    ...
    if (parts.size() != 2) {
        MessageError(tr("The response format is not supported"));
        return;
    }
    ...
    NonBlockingDialogs::getSaveFileName(this, "Save File", filename, "All Files (*.*)",
        [this, content](const QString& filePath) {
            ...
            inputDialog.setWindowTitle(tr("Save agent"));
...
{
    NonBlockingDialogs::getOpenFileName(this, tr("Select file"), QDir::homePath(), "JSON files (*.json)",
        [this](const QString& filePath) {
            ...
            if ( typeIndex == -1 ) {
                MessageError(tr("No such agent exists"));
                return;
            }
...
void DialogAgent::onButtonSave()
{
    ...
    NonBlockingDialogs::getSaveFileName(this, tr("Save File"), tmpFilename, "JSON files (*.json)",
